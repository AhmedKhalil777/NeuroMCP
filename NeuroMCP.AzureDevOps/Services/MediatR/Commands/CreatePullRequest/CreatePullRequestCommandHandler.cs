...
namespace NeuroMCP.AzureDevOps.Services.MediatR.Commands.CreatePullRequest;
            ...
            if (request.WorkItemIds != null && request.WorkItemIds.Any())
            {
                ...
                foreach (var workItemId in request.WorkItemIds)
                {
                    try
                    {
                        ...
                        await witClient.UpdateWorkItemAsync(patchDocument, workItemId, cancellationToken);
                    }
                    catch (Exception ex)
                    {
                        Logger.LogWarning(ex, "Could not link work item {WorkItemId} to PR", workItemId);
                    }
                }
            }
